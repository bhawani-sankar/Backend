...
app.post("/v1/food/order", async (req, res) => {
  ...
  const result = await foodOrder.create({
    order_id,
    name,
    username,
    payment_id,
...
app.get("/v1/food/byname", async (req, res) => {
  ...
  const [result] = await sequelize.query(
    "SELECT * FROM food WHERE type LIKE ? OR FIND_IN_SET(?, type)",
...
app.post("/v1/add/address", async (req, res) => {
  const { username, name, phone, pincode, locality, address, city, state } =
    req.body;
  const data = await Address.findOne({
    where: { username, name, phone, pincode, locality, address, city, state },
  });
  if (data) {
    if (
...
app.get("/v1/food/orders", async (req, res) => {
  const username = req.query.q;
  const result = await foodOrder.findAll({
    where: { username },
    include: food,
  });
  if (!result || result == "") {
